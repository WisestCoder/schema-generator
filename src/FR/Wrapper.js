...
import { copyItem, getKeyFromUniqueId, dropItem } from '../utils/schema';
...
export default function Wrapper({
  ...
  const { selected, hovering, beforeDragComplete } = useGlobalProps();
  const { schema } = item;
  const { type } = schema;
  ...
  const handleMouseLeave = () => {
    // TODO: 如何写hoverLeave，延迟问题，导致无法得到最新的
    // let hoverItem = '';
